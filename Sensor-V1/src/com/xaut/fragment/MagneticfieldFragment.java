...
import com.xaut.entity.Magneticfield;
import com.xaut.sensor_v1.R;
import com.xaut.util.httpUtil;

public class MagneticfieldFragment extends Fragment implements
SensorEventListener{
	...
	private ArrayList<Magneticfield> list = new ArrayList<Magneticfield>();
...
//		setRetainInstance(true);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		view = inflater.inflate(R.layout.magneticfield_layout, null);
		...
		// 获取磁场传感器实例
		magneticfield = sensor.getDefaultSensor(Sensor.TYPE_MAGNETIC_FIELD);
		...
		magshow = (TextView) view.findViewById(R.id.show);

		if (magneticfield != null) {

			magshow.setText("名称：" + magneticfield.getName() + "\n\n版本："
					+ magneticfield.getVersion() + "\n\n供应商："
					+ magneticfield.getVendor() + "\n\n功率："
					+ magneticfield.getPower() + "\n\n分辨率："
					+ magneticfield.getResolution() + "\n\n最大范围："
					+ magneticfield.getMaximumRange() + "\n");
	...
	public void onResume() {
		...
		sensor.registerListener(this, magneticfield, 500000);
	}
	...
	public void onSensorChanged(SensorEvent event) {
		...
		if (flag == 1) {
			...
			magshow.setText("电磁场传感器数据：\n\nx轴磁场：" + Double.parseDouble(df.format(x))
					+ "UT\n\ny轴磁场：" + Double.parseDouble(df.format(y))
					+ "UT\n\nz轴磁场：" + Double.parseDouble(df.format(z)) + "UT\n");
...
//			}	
	   ...
	   private String sample(double x, double y, double z){
			...
			String url = httpUtil.BASE_URL + "/MagneticServlet";
	...
	private void update(double x, double y, double z) {
		...
		list.add(new Magneticfield(x, y, z));
	...
	private void getdata() {
		...
		for (int i = 0; i < NUM; i++) {
			...
			list.add(new Magneticfield(0, 0, 0));
		}
	...
	private void initView() {
		...
		mRenderer.setYTitle("数据(UT)");// 设置y轴的标题
