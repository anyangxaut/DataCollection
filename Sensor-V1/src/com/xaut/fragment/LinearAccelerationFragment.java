...
import com.xaut.entity.LinearAcceleration;
...
public class LinearAccelerationFragment extends Fragment implements SensorEventListener{
	...
	private ArrayList<LinearAcceleration> list = new ArrayList<LinearAcceleration>();
...
//		setRetainInstance(true);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		view = inflater.inflate(R.layout.linearacceleration_layout, null);

		// 从系统服务中获取传感器管理器
		sensor = (SensorManager) getActivity().getSystemService(Context.SENSOR_SERVICE);
		
		// 获取线性加速度传感器实例
		linearAcceleration = sensor.getDefaultSensor(Sensor.TYPE_LINEAR_ACCELERATION);
		...
		linshow = (TextView) view.findViewById(R.id.show);

		if (linearAcceleration != null) {
			
			linshow.setText("名称：" + linearAcceleration.getName()
					+ "\n\n版本：" + linearAcceleration.getVersion()
					+ "\n\n供应商：" + linearAcceleration.getVendor()
		        	+ "\n\n功率：" + linearAcceleration.getPower()
					+ "\n\n分辨率："
					+ linearAcceleration.getResolution()
					+ "\n\n最大范围："
					+ linearAcceleration.getMaximumRange() + "\n");
	...
	public void onResume() {
		...
		sensor.registerListener(this, linearAcceleration, 500000);
	}
	...
	public void onSensorChanged(SensorEvent event) {
		...
		if (flag == 1) {
			...
			linshow.setText("线性加速度传感器数据：\n\nx轴加速度：" + Double.parseDouble(df.format(x))
					+ "m/s^2\n\ny轴加速度：" + Double.parseDouble(df.format(y)) + 
					"m/s^2\n\nz轴加速度：" + Double.parseDouble(df.format(z)) + "m/s^2\n");
...
//			}	
	   ...
	   private String sample(double x, double y, double z){
			...
			String url = httpUtil.BASE_URL + "/LinearAccelerationServlet";
	...
	private void update(double x, double y, double z) {
		...
		list.add(new LinearAcceleration(x, y, z));
	...
	private void getdata() {
		...
		for(int i =0; i < NUM; i++){
			...
			list.add(new LinearAcceleration(0, 0, 0));
		}
